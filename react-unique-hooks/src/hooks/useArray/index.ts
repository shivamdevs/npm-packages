...
export default function useArray<T>(defaultValue: T[] = []): ArrayHook<T> {
    ...
    function findIndex(callback: (element: T, index: number, array: T[]) => boolean) {
        return array.findIndex(callback);
    }

    return { array, set: setArray, push, filter, update, remove, clear, find, findIndex, replace: setArray };
}
