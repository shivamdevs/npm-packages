...
export default function useTranslation<L>(
  translations: Translations,
  defaultLanguage: L | string = "en"
): TranslationHook<L> {
  const [language, setLanguage] = useLocalStorage<L | string>(
    "language",
    defaultLanguage
  );
  const [fallbackLanguage, setFallbackLanguage] = useLocalStorage<L | string>(
    "fallbackLanguage",
    defaultLanguage
  );
  ...
  const translate: TranslationUseHook = (key: string) => {
    ...
    return (
      getNestedTranslation<L>(translations, language, keys) ??
      getNestedTranslation<L>(translations, fallbackLanguage, keys) ??
      key
    );
  };
...
function getNestedTranslation<L>(
  translations: Translations,
  language: L | string = "en",
  keys: string[]
): string | undefined {
  const value = keys.reduce(
    (obj: TranslationKeys | string | undefined, key: string) => {
